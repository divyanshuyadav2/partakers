...
use App\Models\admn_user_orga_rela;
...
class Show extends Component
{
    ...
    private function loadContact()
    {
        try {
            ...
            session()->flash('error', 'Contact not found or has been deactivated.');
            return redirect()->route('contacts.index');
        } catch (\Exception $e) {
            Log::error('Error loading contact: ' . $e->getMessage());
            session()->flash('error', 'An error occurred while loading the contact.');
            return redirect()->route('contacts.index');
        }
    }
...
private function loadDocuments()
{
    try {
        $this->documents = Admn_Docu_Mast::where('Admn_User_Mast_UIN', $this->contactId)
            ->where('Stau', 100201)
            ->get()
            ->groupBy('Regn_Numb') // Groups rows by Reg Number to handle multiple types per doc
            ->map(function ($docGroup) {
                $firstDoc = $docGroup->first(); // Get common details from the first row
                return [
                    'Docu_Name' => $firstDoc->Docu_Name,
                    'Admn_Docu_Mast_UIN' => $firstDoc->Admn_Docu_Mast_UIN,
                    'Regn_Numb' => $firstDoc->Regn_Numb,
                    // Collect all Type IDs into the array for the multi-select
                    ...
                    // Keep track of all IDs in this group for updates/deletes
                    'existing_docs' => $docGroup->pluck('Admn_Docu_Mast_UIN')->toArray(),
                    // CORRECTION HERE: Use $firstDoc, not $first
                    'Prmy' => (bool) ($firstDoc->Prmy ?? false),
                    'is_dropdown_open' => false, // Initialize for UI state
                ];
            })
            ->values()
            ->toArray();
