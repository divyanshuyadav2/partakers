<div>
    <div class="max-w-4xl mx-auto py-10 sm:px-6 lg:px-8">
        ...
        @if ($showNoteSidebar)
            <!-- Backdrop -->
            <div class="fixed inset-0 bg-black/40 backdrop-blur-sm z-40" wire:click="toggleNoteSidebar"></div>

            <div class="fixed inset-0 overflow-hidden z-50">
                <div class="absolute inset-0 overflow-hidden">
                    <!-- Sidebar Container | Right Aligned -->
                    <div wire:click.away="toggleNoteSidebar"
                        class="fixed right-0 inset-y-0 flex flex-col bg-slate-900/95 w-full md:max-w-[400px] shadow-2xl rounded-l-xl transform transition-transform ease-in-out duration-300">

                        <!-- Header -->
                        <header
                            class="bg-slate-900/90 sticky top-0 z-10 px-4 py-3 border-b border-slate-800/50 flex items-center justify-between">
                            <h3 class="text-base font-medium text-white flex items-center gap-2">
                                <i class="bi bi-journal-text text-blue-400"></i> Notes
                            </h3>
                            <button type="button" wire:click="toggleNoteSidebar"
                                class="text-slate-400 hover:text-white transition-colors p-1 rounded-full">
                                <i class="bi bi-x text-lg"></i>
                            </button>
                        </header>

                        <!-- Content -->
                        <div class="flex flex-col flex-1 p-4 gap-3 overflow-hidden min-h-0">
                            <!-- Notes Section -->
                            <div class="flex-[0.3] overflow-y-auto flex flex-col gap-1 min-h-[100px]">
                                @if (count($existingNotes) > 0)
                                    @foreach ($existingNotes as $note)
                                        <div x-data="{ openMenu: false }"
                                            class="group flex items-center justify-between bg-slate-800/50 border border-slate-700/60 hover:border-slate-600 px-3 py-1.5 rounded-md text-sm text-slate-300 transition">
                                            <!-- Note content -->
                                            ...
                                            <div class="relative flex items-center gap-1.5">
                                                <!-- Everyone can unpin -->
                                                @if ($note['isPinned'])
                                                    <button x-on:click="openMenu = false"
                                                        wire:click="unpinNote({{ $note['id'] }})"
                                                        class="p-1 text-yellow-400 hover:text-yellow-300 rounded-full transition"
                                                        title="Unpin Note">
                                                        <i class="bi bi-pin-fill text-xs"></i>
                                                    </button>
                                                @endif

                                                @if ($this->canDeleteNote($note) || !$note['isPinned'])
                                                    <!-- 3-dot -->
                                                    ...
                                                    <!-- Inline menu (background improved for visibility) -->
                                                    <div x-show="openMenu" @click.away="openMenu = false" x-transition
                                                        class="absolute right-full mr-2 flex items-center gap-1 bg-slate-900/80 border border-slate-700/60 rounded-md px-2 py-1 shadow-lg backdrop-blur-sm z-20">
                                                        <!-- Anyone can pin -->
                                                        @if (!$note['isPinned'])
                                                            <button x-on:click="openMenu = false"
                                                                wire:click="pinNote({{ $note['id'] }})"
                                                                class="p-1 text-blue-400 hover:text-blue-300 rounded-full"
                                                                title="Pin Note">
                                                                <i class="bi bi-pin-angle text-[0.7rem]"></i>
                                                            </button>
                                                        @endif

                                                        <!-- Delete visible only if creator & within 48 hours -->
                                                        @if ($this->canDeleteNote($note))
                                                            <button x-on:click="openMenu = false"
                                                                wire:click="deleteNote({{ $note['id'] }})"
                                                                wire:confirm="Are you sure you want to delete this note?"
                                                                class="p-1 text-red-400 hover:text-red-300 rounded-full"
                                                                title="Delete Note">
                                                                <i class="bi bi-trash3 text-[0.75rem]"></i>
                                                            </button>
                                                        @endif
                                                    </div>
                                                @endif
                                ...
                                @else
                                    <div
                                        class="h-full flex items-center justify-center border border-slate-700/60 rounded-md">
                                        <div class="text-center">
                                            <i class="bi bi-journal-x text-slate-600 text-2xl mb-1 block"></i>
                                            <p class="text-slate-400 text-sm">No Notes Yet</p>
                                        </div>
                                    </div>
                                @endif
                            </div>
                            <!-- Note Input Field (with Save/Cancel inside) -->
                            <div class="flex-[0.1] min-h-[90px] relative">
                                <label for="new-note-content" class="sr-only">Add Note</label>

                                <!-- Textarea -->
                                <textarea id="new-note-content" wire:model.live.debounce="newNoteContent" placeholder="Add a note..."
                                    class="w-full h-full resize-none p-3 pr-28 text-sm rounded-md border border-slate-700/60 focus:border-blue-500 bg-slate-800/70 text-white placeholder-slate-500 outline-none transition-colors"></textarea>

                                <!-- Action Buttons (bottom-right corner) -->
                                <div class="absolute bottom-2 right-2 flex items-center gap-2">
                                    <button type="button" wire:click="closeNoteSidebar"
                                        class="px-3 py-1.5 bg-slate-700/60 hover:bg-slate-700/80 text-slate-300 hover:text-white text-xs rounded-md transition">
                                        Cancel
                                    </button>
                                    <button type="button" wire:click="saveNote"
                                        @if (empty($newNoteContent)) disabled @endif
                                        class="px-3 py-1.5 bg-blue-500 hover:bg-blue-600 disabled:bg-slate-700/50 disabled:text-slate-500 text-white text-xs rounded-md transition">
                                        Save
                                    </button>
                                </div>
                            </div>
                            ...
                            <div class="flex-[0.6] flex flex-col min-h-0">
                                <p class="text-sm font-medium text-slate-300 flex items-center gap-1.5 mb-1">
                                    <i class="bi bi-lightning-charge-fill text-yellow-400 text-xs"></i> Quick
                                    Comments
                                </p>
                                        ...
                                        <div class="sm:col-span-2">
                                            <div class="relative">
                                                <input type="tel"
                                                    ...
                                                    class="form-input-figma text-sm w-full @error('phones.' . $index . '.Phon_Numb') border-red-500 @enderror"
                                        ...
                                        <div class="sm:col-span-1">
                                            <div class="relative">
                                                <select wire:model.live="phones.{{ $index }}.Phon_Type"
                                                    class="form-select-figma text-sm w-full @error('phones.' . $index . '.Phon_Type') border-red-500 @enderror">
                                        ...
                                        <div class="sm:col-span-2">
                                            ...
                                            <div class="relative">
                                                <input type="tel" id="landline_number_{{ $index }}"
                                                    ...
                                                    class="form-input-figma w-full text-sm @error('landlines.' . $index . '.Land_Numb') border-red-500 @enderror"
                                        ...
                                        <div class="sm:col-span-1">
                                            <div class="relative">
                                                <select id="landline_type_{{ $index }}"
                                                    ...
                                                    class="form-select-figma w-full text-sm @error('landlines.' . $index . '.Land_Type') border-red-500 @enderror">
                                                    <option value="home">Home</option>
                                                    <option value="office">Office</option>
                                                </select>
                            ...
                            @forelse ($emails as $index => $email)
                                <div class="relative p-4 rounded-md bg-slate-700/30 border border-slate-600"
                                    ...
                                    <div class="flex-grow grid grid-cols-1 sm:grid-cols-3 gap-3">
                                        <div class="sm:col-span-2">
                                            <div class="relative">
                                                <input type="email"
                                                    ...
                                                    class="form-input-figma text-sm w-full lowercase @error('emails.' . $index . '.Emai_Addr') border-red-500 @enderror">
                                        ...
                                        <div class="relative">
                                            <select wire:model.live="emails.{{ $index }}.Emai_Type"
                                                class="form-select-figma text-sm w-full @error('emails.' . $index . '.Emai_Type') border-red-500 @enderror">
                        ...
                        <div class="p-4 sm:p-5 rounded-md bg-slate-700/30 border border-slate-600"
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                                ...
                                <div class="col-span-1 sm:col-span-2">
                                    ...
                                    <!-- Display newly selected file info (before save) -->
                                    ...
                                    @elseif (!empty($document['existing_file_path']) && !is_object($document['Docu_Atch_Path']))
                                        <!-- Display existing file info (after save) -->
                                        <div
                                            ...
                                            <div class="flex items-center gap-2 flex-1 min-w-0">
                                                ...
                                                <div class="flex-1 min-w-0">
                                                    <a href="{{ Storage::url($document['existing_file_path']) }}"
                                                        target="_blank"
                                                        class="font-medium truncate hover:text-blue-400 hover:underline"
                                                        title="{{ basename($document['existing_file_path']) }}">
                    ...
                    cropImage() {
                        ...
                        try {
                            ...
                            canvas.toBlob((blob) => {
                                ...
                                this.$wire.upload('Prfl_Pict', file,
                                    (uploadedFilename) => {
                                        console.log('Image uploaded successfully:', uploadedFilename);
                                        this.isUploading = false;

                                        // Wait a moment for Livewire to update, then close
                                        this.$nextTick(() => {
                                            setTimeout(() => {
                                                this.closeCropper();
                                                // Dispatch custom event to refresh image
