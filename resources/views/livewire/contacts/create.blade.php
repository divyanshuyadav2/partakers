                                ...
                                <div
                                    ...
                                    <div
                                        ...
                                        @foreach ($this->getQuickComments() as $category => $comments)
                                            @php $displayCat = trim($category); @endphp

                                            <button type="button"
                                                wire:click="$set('activeCommentTab', @js($displayCat))"
                                                class="px-2.5 py-1 text-xs font-medium rounded-sm transition-colors whitespace-nowrap
                {{ $activeCommentTab === $displayCat
                    ...
                    : 'text-slate-400 hover:bg-slate-700/30 hover:text-slate-200' }}">
                                    ...
                                    <div class="flex-1 overflow-y-auto p-1.5 space-y-1">
                                        @foreach ($this->getQuickComments()->filter(fn($_, $key) => trim($key) === $activeCommentTab)->first() ?? collect() as $comment)
                                            <button type="button"
                                                wire:click="addCommentToNote(@js($comment->Comnt_Text))"
                                                class="w-full px-2 py-1.5 text-xs bg-slate-700/40 hover:bg-blue-500/15
                   text-slate-300 hover:text-blue-300 rounded-sm transition-colors
                   text-left truncate">
                                                {{ $comment->Comnt_Text }}
                                            </button>
                                        @endforeach
