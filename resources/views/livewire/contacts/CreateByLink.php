...
use App\Livewire\Traits\{WithDocumentNames,HasMaxConstants,HasWorkTypes,HasSkillData,WithContactValidation };
use App\Models\{
    Admn_Addr_Type_Mast,
    Admn_Bank_Name,
    Admn_Cnta_Link_Mast,
...
{
    use WithFileUploads;
    use WithDocumentNames, HasMaxConstants, HasSkillData, HasWorkTypes, WithContactValidation;
    ...
    // const MAX_BANK_FILE_SIZE = 102400;  // 100 KB
    // const MAX_DOCUMENT_FILE_SIZE = 204800;  // 200 KB (Synced with Create.php)
    // const MAX_PROFILE_FILE_SIZE = 2048 * 1024;  // 2 MB
   ...
   public function updated($propertyName)
    ...
    private function loadReferenceData(): void
    {
        ...
        $this->allCountries = Admn_Cutr_Mast::select('Admn_Cutr_Mast_UIN', 'Name', 'Code', 'Phon_Code', 'MoNo_Digt')
            ->where(
                fn($q) => $q
        ...
        $this->bankOptions = Admn_Bank_Name::select('Bank_UIN', 'Bank_Name')
            ->orderBy('Bank_Name')
            ->get();

        $this->allDocumentTypes = Admn_Docu_Type_Mast::where('Stau', self::STATUS_ACTIVE)->orderBy('Docu_Name')->get();
    }
    ...
    public function updatedAddresses($value, $key): void
    {
        ...
        match ($field) {
            'Admn_Cutr_Mast_UIN' => $this->handleCountryChange($addr, $value),
            'Admn_Stat_Mast_UIN' => $this->handleStateChange($addr, $value),
            'Admn_Dist_Mast_UIN' => $this->handleDistrictChange($addr, $value),
            'pincodeSearch' => $this->handlePincodeSearch($addr, $value),
            default => null
        };
    }
    ...
    {
        try {
            $pincode = Admn_PinCode_Mast::with('district.state.country')->find($pincodeUIN);
            ...
            $addr['statesForDropdown'] = $addr['Admn_Cutr_Mast_UIN'] ? Admn_Stat_Mast::where('Admn_Cutr_Mast_UIN', $addr['Admn_Cutr_Mast_UIN'])
                ->orderBy('Name')
                ->get()
                ->toArray() : [];

            $addr['districtsForDropdown'] = $addr['Admn_Stat_Mast_UIN'] ? Admn_Dist_Mast::where('Admn_Stat_Mast_UIN', $addr['Admn_Stat_Mast_UIN'])
                ->orderBy('Name')
                ->get()
                ->toArray() : [];
            ...
            Log::error('Error selecting pincode: ' . $e->getMessage());
        }
    }
    ...
    private function hydrateAddressFields($data, $id = null)
    {
        ...
        return array_merge($data, [
            ...
            'statesForDropdown' => $countryId ? Admn_Stat_Mast::where('Admn_Cutr_Mast_UIN', $countryId)->orderBy('Name')->get()->toArray() : [],
            'districtsForDropdown' => $stateId ? Admn_Dist_Mast::where('Admn_Stat_Mast_UIN', $stateId)->orderBy('Name')->get()->toArray() : [],
            'pincodesForDropdown' => $distId ? Admn_PinCode_Mast::where('Admn_Dist_Mast_UIN', $distId)
                ->orderBy('Code')
                ->select('Admn_PinCode_Mast_UIN', 'Code')
                ->get()
                ->toArray() : [],
    ...
    private function generateUniqueUIN($table, $pkColumn): int
    {
        for ($i = 0; $i < 100; $i++) {
        ...
        throw new \Exception("Failed to generate unique UIN after 100 attempts for table: {$table}");
    }

    private function generateBatchUINs($table, $pkColumn, $count): array
    {
        $lastUIN = DB::table($table)->max($pkColumn);
        $startUIN = (!$lastUIN || $lastUIN < self::BASE_UIN) ? self::BASE_UIN : $lastUIN + 1;
    ...
    private function formatDateForDatabase($value): ?string
    {
        if (empty($value)) {
            return null;
        }

        // If it is already a Carbon object, just format it
        if ($value instanceof Carbon) {
            return $value->format('Y-m-d');
        }

        try {
            // Strictly parse the Y-m-d format coming from HTML date inputs
            return Carbon::createFromFormat('Y-m-d', $value)->format('Y-m-d');
        } catch (\Exception $e) {
            Log::warning("Date parsing failed for value: " . json_encode($value));
            return null;
        }
    }
