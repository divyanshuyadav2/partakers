...
class ContactExportController extends Controller
{
    ...
    public function exportCsv(Request $request)
    {
        ...
        // ============================================================
        // Eager load relationships - FIXED: Load all bank accounts and documents
        // ============================================================
        $contacts = $query->with([
            'tags',
            'prefix',
            ...
            'skills',
            'currentEmployment',
            'bankAccounts' => fn($q) => $q->orderBy('Prmy', 'desc')->with('bank'),  // ✅ Get all, ordered by primary
            'documents' => fn($q) => $q->orderBy('Prmy', 'desc')->with('documentType'),  // ✅ Get all, ordered by primary
            'group',
        ])->get();
        ...
        $callback = function () use ($contacts, $selectedColumns, $allColumnLabels) {
            ...
            foreach ($contacts as $contact) {
                ...
                // ------------------------------------------------
                // BANK ACCOUNT - FIXED: Get primary if exists, otherwise first available
                // ------------------------------------------------
                $bankAccounts = $contact->bankAccounts;
                $primaryBank = $bankAccounts->firstWhere('Prmy', 1) ?? $bankAccounts->first();
                ...
                // ------------------------------------------------
                // DOCUMENT - FIXED: Get primary if exists, otherwise first available
                // ------------------------------------------------
                $documents = $contact->documents;
                $primaryDoc = $documents->firstWhere('Prmy', 1) ?? $documents->first();
                
                $docName = optional($primaryDoc)->Docu_Name ?? '';
                $docType = optional(optional($primaryDoc)->documentType)->Docu_Name ?? '';
                $docNumber = optional($primaryDoc)->Regn_Numb ?? '';  // ✅ Fixed: Use Regn_Numb
